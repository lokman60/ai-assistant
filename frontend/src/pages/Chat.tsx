    ...
    <div className="flex h-full">
      <div className="w-72 border-r border-gray-200 bg-white p-4 flex flex-col">
        ...
        <div className="mb-4">
          ...
          {readyDocs.length === 0 && <p className="text-xs text-gray-400 px-3">No processed documents</p>}
        </div>
          ...
          {convs.map((conv: any) => (
            <div key={conv.id} className="flex items-center group">
              <button onClick={() => setSearchParams({ conv: conv.id })}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm flex-1 text-left transition-colors ${conv.id === convId ? 'bg-gray-100 text-gray-900' : 'text-gray-600 hover:bg-gray-50'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && !chatMutation.isPending && (
            <div className="text-center text-gray-400 mt-20">
              ...
              <h2 className="text-xl font-semibold text-gray-600 mb-1">Ask about your documents</h2>
        ...
        {sources.length > 0 && (
          <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
            <div className="flex items-center gap-2 text-xs text-gray-500">
              ...
              {sources.map((s, i) => (
                <span key={i} className="bg-white px-2 py-1 rounded border border-gray-200 text-xs">
