  ...
  return (
    <div className="flex h-full">
      <div className="w-72 border-r border-gray-200 dark:border-border-dark bg-white dark:bg-surface-card-dark p-4 flex flex-col">
        ...
        <div className="mb-4">
          <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Documents</h3>
          <div className="space-y-1 max-h-40 overflow-y-auto">
            {readyDocs.map((doc: any) => (
              <label key={doc.id} className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-sm transition-colors ${selectedDocs.includes(doc.id) ? 'bg-pdf-50 dark:bg-pdf-900/20 text-pdf-700' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900'}`}>
          ...
          {readyDocs.length === 0 && <p className="text-xs text-gray-500 dark:text-gray-400 px-3">No processed documents</p>}
        </div>
        <h3 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Conversations</h3>
        <div className="flex-1 overflow-y-auto space-y-1">
          {convs.map((conv: any) => (
            <div key={conv.id} className="flex items-center group">
              <button onClick={() => setSearchParams({ conv: conv.id })}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm flex-1 text-left transition-colors ${conv.id === convId ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {messages.length === 0 && !chatMutation.isPending && (
            <div className="text-center text-gray-500 dark:text-gray-400 mt-20">
              ...
              <h2 className="text-xl font-semibold text-gray-600 dark:text-gray-400 mb-1">Ask about your documents</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-gray-200 dark:border-border-dark bg-white dark:bg-surface-card-dark">
          <div className="flex gap-2 mb-2">
            <div className="relative">
              <button type="button" onClick={() => setShowActions(!showActions)}
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 transition-colors">
                ...
                <div className="absolute bottom-full mb-1 left-0 bg-white dark:bg-surface-card-dark border border-gray-200 dark:border-border-dark rounded-lg shadow-lg z-10 w-48">
                  {ACTIONS.map(a => (
                    <button key={a.value} type="button" onClick={() => { setAction(a.value); setShowActions(false) }}
                      ...
                      <div className="text-gray-500 dark:text-gray-400 truncate">{a.desc}</div>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="flex gap-3 items-center">
            <input
              ...
              className="flex-1 px-4 py-3 bg-white dark:bg-surface-card-dark border border-gray-300 dark:border-gray-600 rounded-xl text-text-primary dark:text-text-primary-dark placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-pdf-500 focus:border-pdf-500 outline-none transition-colors disabled:bg-gray-50 dark:disabled:bg-gray-800 disabled:text-gray-400"
            />
