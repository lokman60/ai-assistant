...
export default function UploadPage() {
  ...
  const uploadMutation = useMutation({
    ...
    onError: (err: any) => {
      const msg = err?.message || err?.response?.data?.message || err?.response?.data?.detail || 'Upload failed'
      setUploadError(msg)
    },
  })
    ...
    <div className="p-8">
      ...
      <div
        ...
        className={`border-2 border-dashed rounded-xl p-12 text-center cursor-pointer transition-colors mb-8 ${
          isDragActive ? 'border-pdf-500 bg-pdf-50 dark:bg-pdf-900/20' : 'border-gray-300 dark:border-gray-600 hover:border-pdf-400 hover:bg-gray-50 dark:hover:bg-gray-800 bg-gray-50 dark:bg-gray-900'
        }`}
      >
        <input {...getInputProps()} />
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">or click to browse (max 50MB, PDF only)</p>
      </div>

      {uploadMutation.isPending && (
        <div className="bg-pdf-50 dark:bg-pdf-900/20 text-pdf-700 dark:text-pdf-400 p-4 rounded-lg mb-6 flex items-center gap-3">
      ...
      {uploadError && (
        <div className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 p-4 rounded-lg mb-6 flex items-center gap-3">
      ...
      <div className="bg-white dark:bg-surface-card-dark rounded-xl border border-gray-200 dark:border-border-dark shadow-sm">
        ...
        {isLoading ? (
          ...
          <div className="p-6 text-center text-gray-500 dark:text-gray-400">No documents uploaded yet</div>
        ) : (
                  ...
                  {editingId === doc.id ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        className="px-2 py-1 bg-white dark:bg-surface-card-dark border border-gray-300 dark:border-gray-600 rounded text-sm text-text-primary dark:text-text-primary-dark focus:ring-2 focus:ring-pdf-500 outline-none"
                        autoFocus
                      />
                      ...
                      <button onClick={() => setEditingId(null)} className="text-gray-500 dark:text-gray-400 hover:text-gray-600">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ) : (
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">{formatDistanceToNow(new Date(doc.created_at))}</p>
                    </div>
                  )}
                </div>
